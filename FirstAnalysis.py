...
def get_rainfall_arrival_ride(arrival_ride_="",f=np.mean):
    return([f([annual_rainfall[year] for year in range(1902,2013) if rides_data[str(year)][0]==arrival_ride_]),
    f([annual_rainfall[year] for year in range(1902,2013) if rides_data[str(year)][0]!=arrival_ride_])]
                    )
